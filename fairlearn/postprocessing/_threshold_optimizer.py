...
class ThresholdOptimizer(BaseEstimator, MetaEstimatorMixin):
    ...
    def fit(self, X, y, *, sensitive_features, batch_size=None, device='cpu', **kwargs):
        """Fit the model.
        ...
        if batch_size is None:
            ...
            scores = _get_soft_predictions_batch(self.estimator_, X, self._predict_method, batch_size=batch_size, device=device)
    ...
    def predict(self, X, *, sensitive_features, random_state=None, batch_size=None, device='cpu'):
        ...
        preds = self.interpolated_thresholder_.predict(
            X, sensitive_features=sensitive_features, random_state=random_state, batch_size=batch_size, device=device
        )
        return preds
