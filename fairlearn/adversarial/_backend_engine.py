...
class BackendEngine():
    ...
    def __init__(self, base, X, Y, Z):
        ...
        if isinstance(base.predictor_model, list):
            predictor_list_nodes = [X.shape[1]] + base.predictor_model + [Y.shape[1]]
            self.predictor_model = self.get_model(list_nodes=predictor_list_nodes)
            ...
            raise ValueError(_KWARG_ERROR_MESSAGE.format(
                "predictor_model", "a list or a \'{}\'".format(self.model_class)))

        # Set up adversary_model
        if isinstance(base.adversary_model, list):  
            adversarial_in = Y.shape[1] * (2 if base.pass_y_ else 1)
            adversary_list_nodes = [adversarial_in] + base.adversary_model + [Z.shape[1]]
            self.adversary_model = self.get_model(list_nodes=adversary_list_nodes)
        elif issubclass(type(base.adversary_model), self.model_class):
            self.adversary_model = base.adversary_model
        else:
            raise ValueError(_KWARG_ERROR_MESSAGE.format(
                "adversary_model", "a list or a \'{}\'".format(self.model_class)))
        ...
        self.adversary_loss = self.get_loss(
            interpret_keyword(Z, base.adversary_loss, "adversary_loss", "sensitive_features"))
