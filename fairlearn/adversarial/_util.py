from ._constants import _KWARG_ERROR_MESSAGE, \
    _TYPE_CHECK_ERROR, _TYPE_COMPLIANCE_ERROR, _INVALID_DATA, \
...
def interpret_keyword(data, input_type, param_name, data_name):
        ...
        raise ValueError(_KWARG_ERROR_MESSAGE.format(
            param_name, "one of {} or a callable".format([kw.value for kw in Keyword])))
...
def check_and_infer_type(Y, keyword, data_name):
    ...
    if inferred == 'multilabel-indicator':
        ...
        if not np_all(np_sum(Y, axis=1) == 1):
            raise ValueError(_TYPE_CHECK_ERROR.format(data_name, Keyword.CATEGORY.value))

    # Match inferred with keyword
    if inferred == 'binary' and \
        keyword in [Keyword.BINARY, Keyword.CLASSIFICATION, Keyword.AUTO]:
        return Keyword.BINARY
    elif inferred == 'multilabel-indicator' and \
        keyword in [Keyword.CATEGORY, Keyword.CLASSIFICATION, Keyword.AUTO]:
        return Keyword.CATEGORY
    elif inferred in ['continuous', 'continuous-multioutput'] and \
        keyword in [Keyword.CONTINUOUS, Keyword.AUTO]:
        return Keyword.CONTINUOUS

    raise ValueError(_TYPE_COMPLIANCE_ERROR.format(
        data_name, keyword.value, inferred))
...
def _get_function(keyword):
    ...
    if keyword == Keyword.BINARY:
        ...
        raise ValueError(_PREDICTION_FUNCTION_AMBIGUOUS.format(
