...
def _validate_and_reformat_input(
    ...
    The :code:`**kwargs` can contain :code:`sensitive_features=`
    and :code:`control_features=`
    parameters.
    ...
    Tuple(pandas.DataFrame, pandas.Series, pandas.Series, pandas.Series)
        The validated and reformatted X, y, sensitive_features and
        control_features; note
        that certain estimators rely on metadata encoded in X which may
        be stripped during
        the reformatting process, so mitigation methods should ideally
        use the input X instead
        of the returned X for training estimators and leave potential
        reformatting of X to the
        estimator.

    """
    result_y, result_sf, result_cf = _validate_and_reformat_labels_and_sf(
        y, expect_y, enforce_binary_labels, **kwargs
    )
    if y is not None:
...
def _validate_and_reformat_labels_and_sf(
    y=None, expect_y=True, enforce_binary_labels=False, **kwargs
):
    """
    ...
    Tuple(pandas.Series, pandas.Series, pandas.Series)
        The validated and reformatted y, sensitive_features
        and control_features.
    """
    ...
    if control_features is not None:
        ...
        control_features = check_array(
            control_features, ensure_2d=False, dtype=None
        )
...
def _validate_and_reformat_labels(
    y=None, expect_y=True, enforce_binary_labels=False, **kwargs
):
    """
    Validate labels and sensitive features data and return the data in an appropriate format.

    The :code:`**kwargs` can contain :code:`sensitive_features=` and
    :code:`control_features=`
    parameters.
    ...
    y : numpy.ndarray, pandas.DataFrame, pandas.Series, or list
        The label vector
    expect_y : bool
        If True y needs to be provided, otherwise ignores the
    ...
    """
    if y is not None:
        # calling check_X_y with a 2-dimensional y causes a warning,
        # so ensure it is 1-dimensional
        ...
        if enforce_binary_labels and not set(np.unique(y)).issubset(
            set([0, 1])
        ):
        ...
        .apply(
            lambda row: _MERGE_COLUMN_SEPARATOR.join(
                [
                    ...
                    .replace(
                        _MERGE_COLUMN_SEPARATOR, "\\" + _MERGE_COLUMN_SEPARATOR
                    )
