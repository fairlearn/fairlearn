...
class OptimalTransport(BaseEstimator, TransformerMixin):
    ...
    def fit(self, X, y=None):
        ...
        assert isinstance(X, np.ndarray) or isinstance(
            X, pd.DataFrame
        ), "X must be a np.ndarray or pd.DataFrame."
        assert isinstance(y, np.ndarray) or isinstance(
            y, pd.DataFrame
        ), "y must be a np.ndarray or pd.DataFrame."
        ...
        if self.transport_method == "sinkhorn":
            gamma = ot.sinkhorn(
                marginal_x, marginal_y, dist, self.sinkhorn_regularization
            )
    ...
    def transform(self, X):
        ...
        """
        assert hasattr(self, "gamma") and hasattr(
            self, "y"
        ), "You must call .fit() before transforming a variable."
        assert isinstance(X, np.ndarray) or isinstance(
            X, pd.DataFrame
        ), "X must be a np.ndarray or pd.DataFrame."
        ...
        for i in range(len(source)):
            transported[i, :] = np.sum(self.gamma[i, :] * self.y.T, axis=1) / np.sum(
                self.gamma[i, :]
            )  # Compute average Yi
