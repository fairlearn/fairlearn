...
* Fixed ``AssertionError: data can be loaded only once`` when reusing a constraints object
  across multiple calls to ``GridSearch.fit`` or ``ExponentiatedGradient.fit`` by
  deep-copying constraints before loading data. The original constraints object remains
  unloaded, and the fitted copy is available as ``constraints_``. Also resolves associated
  ``PerformanceWarning`` due to DataFrame fragmentation: :pr:`1632` by
  :user:`BALOGUN-DAVID <BALOGUN-DAVID>`.
