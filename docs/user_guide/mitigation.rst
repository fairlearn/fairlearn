   ...
   *  - :class:`~fairlearn.adversarial.AdversarialFairness`
      - Inprocessing algorithm based on the paper *Mitigating Unwanted Biases*
        *with Adversarial Learning* [#4]_. This algorithm uses a predictor and an adversarial neural network, defined either as a `PyTorch module
        ...
        `Tensorflow2 model 
        <https://www.tensorflow.org/api_docs/python/tf/keras/Model>`_. Must have transformed data to 2d :class:`numpy.ndarray` of type :class:`float`.
      - ✔
      - ✔
      - DP, EO
...
To summarize [#4]_, assume we have data :math:`X, Y, Z`, where we want to
...
This model can be trained for either *demographic parity* or *equalized odds*. TODO

Firstly, we will dicuss the models that this implementation accepts in :ref:`models`.
Secondly, in :ref:`data_preprocessing`, we discuss the required data preprocessing, as :math:`X, Y, Z` must be arrays of
floats, though we might want to support multiclass classification. Then, in :ref:`loss_functions`,
we describe what are good choices of :math:`L_P` and :math:`L_A`. Lastly, in :ref:`training` we give some
useful tips to keep in mind when training this model, as adversarial methods such as these
...
For simple or exploratory use cases, Fairlearn provides a very basic neural network constructor.
Instead of a neural network model, we can pass a list of numbers :math:`h_1, h_2, \dots` that indicate
the number of nodes :math:`h_i` per hidden layer :math:`i`. The number of nodes in the input
and output layer are automatically inferred from data, as soon as data is passed. 
...
Though, we
do recommend you create your own neural networks beforehand, as you can tweak these to your
liking.
...
In contrast to other mitigation techniques, we require you to preprocess your data
to matrices of floats. It is very typical to be dealing with data other than numbers,
such as categorical data, and in those cases we leave it to the user to preprocess this
to numbers. The user can make the decision on how to encode this column, for instance
using a one-hot-encoding or by mapping the values to an integer range. Another reason
that we do not provide such a data preprocessor out of the box, is that scikit-learn makes
preprocessing easy. For instance, say we want to transform the following dataset
...
We want to encode the categorical data using one-hot encodings, and we want to scale
the other features to a standard range. An easy way to do this, using sci-kit learn, is
to create a `ColumnTransform` as such.
...
Similarly to other methods, there are more common aspects to preprocessing besides
encoding everything with numbers. In particular, one needs to take care in handling NaN's,
as NaN's are not supported by PyTorch or TensorFlow. Also, as for other neural network
...
The loss functions of this method [#4]_, denoted by :math:`L_P` and :math:`L_A`, are not defined explicitely. To provide the most general interface, you can pass your own loss functions. This can be either PyTorch or TensorFlow loss functions, depending on which module the neural networks are. For instance, in the following example we set the `predictor_loss` to a PyTorch-implemented binary cross-entropy, and the `adversary_loss` to the mean squared error. Additionally, we explicitely specify how the **discrete predictions** are computed, by providing a `predictor_function`. In this case, the predictor function maps sigmoid logits to the discrete prediction.
...
*Note that the PyTorch and TensorFlow modules switch the order of arguments in their loss functions, so be extra careful when defining custom loss functions so that you adhere to the module-specific argument order*

We do, however, offer some shortcuts for binary, categorical, and continuous data. Namely, instead of passing a function, you can pass any of the keywords defined in `fairlearn.adversarial.Keyword` to signal a certain distribution type and use a certain PyTorch or TensorFlow specific loss function. For instance, the above example is equivalent to passing the keywords :code:`predictor_loss='binary'`, :code:`adversary_loss='continuous'` and :code:`predictor_function='binary'`. The default argument for any of these three keywords is :code:`'auto'`, which automatically infers the distribution type. *Note: distribution types are validated on the first batch of data that the model sees, so a* :code:`'binary'` *assumption on non-binary data will raise an error.*

Theoretical results of [#4]_ show that under some strong assumptions, the predictor will satisfy the fairness constraint: demographic parity or equality of odds.

In particular, to state the results of *Proposition 2* of [#4]_ regarding demographic parity, if we assume: 

#. The variables :math:`Y` and :math:`Z` have underlying distributions :math:`D_Y` and :math:`D_Z` respectively.
#. The adversary is strong enough that, at convergence, it has learned a function that minimizes the cross-entropy loss.
#. The predictor completely fools the adversary. In other words, the loss of the adversarial is maximized and equal to the entropy of :math:`Z`.

Then, the predictor satisfies demographic parity. *This tells us that choosing cross-entropy loss for the adversary makes sense!*

The specific cross-entropy loss is then still dependent on the assumption we make on the distribution :math:`D_Z`. If we assumed :math:`D_Z` follows a Bernoulli distribution, then binary cross-entropy may be sensible. If we assume :math:`D_Z` is normally distributed, then cross-entropy loss between two normal distributions is most sensible. Practically, mean squared error loss is a sensible choice too, as it is proportional to the cross-entropy of two normal distributions.
...
- Slow Training
- alpha parameter
- changing parameters during training makes sense
