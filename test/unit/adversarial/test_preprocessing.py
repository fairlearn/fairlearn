...
def data_generator():
    ...
    yield [0.1, 2., 0.999999, 100000.1], 'continuous'
...
def checker(data, dist_type):
    ...
    assert isinstance(is_equal, bool) and is_equal or \
        isinstance(is_equal, ndarray) and is_equal.all() or \
        isinstance(is_equal, Series) and is_equal.all() or \
        isinstance(is_equal, DataFrame) and is_equal.all().all()
