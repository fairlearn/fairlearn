...
class Test1m1sf0cf:
    ...
    def _prepare(self, metric_fn):
        self.target = metrics.MetricFrame(metric_fn, y_t, y_p, sensitive_features=g_4)
...
class Test1m1sf0cfFnDict:
    ...
    def _prepare(self, metric_fn):
        ...
        self.target = metrics.MetricFrame(
            {self.mfn: metric_fn}, y_t, y_p, sensitive_features=g_4
        )
...
class Test2m1sf1cf:
    def _prepare(self):
        fns = {"recall": skm.recall_score, "prec": skm.precision_score}
        self.target = metrics.MetricFrame(
            fns, y_t, y_p, sensitive_features=g_2, control_features=g_3
        )
