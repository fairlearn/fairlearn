...
class TestOverallQuantiles:
    ...
    def test_1mdict_0cf(self, mf_1mdict_0cf):
        quantiles = [0.2, 0.5, 0.9]
        overall_quantiles = mf_1mdict_0cf.overall_quantiles(quantiles)
        assert isinstance(overall_quantiles, pd.Series)
        assert len(overall_quantiles) == 1
        assert isinstance(overall_quantiles["recall"], np.ndarray)
        assert overall_quantiles["recall"].shape == (len(quantiles),)
        # Overall value should be close to quantile 0.5
        assert mf_1mdict_0cf.overall["recall"] == pytest.approx(
            overall_quantiles["recall"][1], abs=0.05
        )
