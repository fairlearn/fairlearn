...
def test_basic(transform_y_t, transform_y_p):
    ...
    target = metrics.MetricFrame(
        skm.recall_score, transform_y_t(y_t), transform_y_p(y_p), sensitive_features=g_f
    )
...
def test_2m_1sf_sample_weights():
    ...
    target = metrics.MetricFrame(
        metrics_dict, y_t, s_w, sensitive_features=g_2, sample_params=sample_params
    )
...
def test_duplicate_sf_names():
    ...
    with pytest.raises(ValueError) as execInfo:
        _ = metrics.MetricFrame(skm.recall_score, y_t, y_p, sensitive_features=groups)
...
def test_duplicate_cf_names():
    ...
    with pytest.raises(ValueError) as execInfo:
        _ = metrics.MetricFrame(
            skm.recall_score, y_t, y_p, sensitive_features=g_4, control_features=groups
        )
    assert execInfo.value.args[0] == msg
...
def test_duplicate_cf_sf_names():
    ...
    with pytest.raises(ValueError) as execInfo:
        _ = metrics.MetricFrame(
            skm.recall_score, y_t, y_p, sensitive_features=sf, control_features=cf
        )
    assert execInfo.value.args[0] == msg
...
def test_single_element_lists():
    mf = metrics.MetricFrame(
        skm.balanced_accuracy_score, [1], [1], sensitive_features=[0]
    )
    assert mf.overall == 1
