...
def run_comparisons(moment, metric_fn):
    ...
    mf_input = MetricFrame(
        metric_fn, y, y, sensitive_features=X["sens"], control_features=X["ctrl"]
    )
    ...
    mf_unmitigated = MetricFrame(
        metric_fn, y, y_pred, sensitive_features=X["sens"], control_features=X["ctrl"]
    )
...
def test_equalized_odds():
    ...
    y_pred = unmitigated.predict(X_dummy)
    mf_unmitigated = MetricFrame(
        metrics, y, y_pred, sensitive_features=X["sens"], control_features=X["ctrl"]
    )
