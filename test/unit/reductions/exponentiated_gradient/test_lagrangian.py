...
def test_lagrangian_eval(eps, Constraints, use_Q_callable, opt_lambda):
    ...
    assert (
        np.isclose(
            fitted_estimator.weights, expected_estimator_weights[task_type], atol=1.0e-6
        )
    ).all()
...
def test_call_oracle(Constraints, eps, estimator, mocker):
    ...
    lagrangian = _Lagrangian(
        X, y, estimator, deepcopy(constraints), 1 / eps, sensitive_features=A
    )
...
@pytest.mark.parametrize(
    "Constraints", [DemographicParity, EqualizedOdds, ErrorRateParity, BoundedGroupLoss]
)
@pytest.mark.parametrize("eps", [0.001, 0.01, 0.1])
