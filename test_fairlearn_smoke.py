...
class TestFairLearnSmoke:
    ...
    def run_smoke_test(self, data):
        res_tuple = red.expgrad(self.dataX, self.dataA, self.dataY, self.learner,              
                         		constraints=data["cons_class"](), eps=data["eps"])
        ...
        assert res["best_gap"] == pytest.approx(data["best_gap"], abs=self._PRECISION)
        assert res["last_t"] == data["last_t"]
        assert res["best_t"] == data["best_t"]
        assert res["disp"] == pytest.approx(data["disp"], abs=self._PRECISION)
        assert res["error"] == pytest.approx(data["error"], abs=self._PRECISION)
