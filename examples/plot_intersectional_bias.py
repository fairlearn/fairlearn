...
# ===================
#
# There are 3 general pillars of fairness as it pertains to ML: Transparency, Inclusion, and Impartiality :footcite:`sikstrom2022conceptualising`
#
...
#
# Although all aspects of fairness are equally important, this tutorial will focus on impartiality. Specifically, we aim to examine fairness through the scope of intersectionality, which was originally coined by Kimberle Crenshaw :footcite:`crenshaw1991intersectionality`:
#
...
#
# The key question in being able to do so is this: **Who and what is a population?** It may seem like this question is trivial, and has a clear-cut meaning in no need of further clarification. However, researchers like Nancy Krieger have pointed out that a clear notion of "population" is rarely defined :footcite:`krieger2012population`, despite its centrality to fields like ML fairness.
#
...
#
# In 2011, a paper studying the epidemiology of cancer noted that "early onset ER negative tumors also develop more frequently in Asian Indian and Pakistani women and in women from other parts of Asia, although not as prevalent as it is in West Africa." :footcite:`wallace2011interactions`
#
...
#
# This idea that humans have **innate and comparable** was largely derived from  Adolphe Quetelet's invention of the **"Average Man,"** :footcite:`eknoyan2007adolphe` establishing the notion of a population mean.
#
...
#
# Crucially, we need to be careful about how we define a population, because it can impact the results we get from any analyses - perhaps it might skew them, or perhaps the lack of appropriate comparison groups might render an analysis inappropriate. Let's look an example of this from a 2016 BBC article :footcite:`bbc2016dutch` which compares body heights:
#
# .. image:: ../_static/images/imhc_bodyheight.png
#   :width: 600
#   :alt: A visualization that shows the average height of different countries in 1914 and 2014. The Netherlands has the tallest average body height for men, Iran the biggest height gain for men, South Korea the biggest height gain for women, and Guatamala the smallest average body height for woman.
#
...
# 
# This article is hardly unique in this problem - body mass index (BMI) has become a ubiquitous way of defining obesity, but comparing individuals on the basis of their BMI is problematic: the scale was built by and for white populations, leading to an overestimation of health risks for Black individuals :footcite:`endocrine2009widely`, and an underestimation of health risks for asian individuals :footcite:`racette2003obesity`. Even more interestingly, BMI was never meant to be a way of measuring health at an individual level. :footcite:`psychologytoday2016AdolpheQuetelet`
# 
# ---
# 
...
# 
# Treviranus points out that the further you get from a population mean, the more heterogeneous the population gets. And as you move away from this "average mean" - not only do people get more diverse from one another, but solutions that are designed with the population mean in mind are increasingly ineffective. :footcite:`treviranus2019InclusiveDesign``
# 
# .. image:: ../_static/images/imhc_design.png
#   :width: 400
#   :alt: A visualization that shows scattered dots. The further you move away from the middle, the more scattered the points are. Around the points in the center, a blue circle is drawn that points to 'design works'. Moving further from the center, a second yellow circle points to 'design is difficult to use'. Moving even further from the center, a third orange circle points to 'can't use design'.
#
...
# **Proposition 2**
#
# Given these limitations, let's consider an alternative understanding of a population that :footcite:ct:`krieger2012population` proposes.
# 
...
# 
# Krieger's view emphasizes that **identity is not a fixed attribute (like a star), but rather a fluid social process**. A great example that illustrates the degree to which identity can change can be seen in the racial identity of Puerto Ricans in the aftermath of Hurricane Maria. :footcite:`godreau2021nonsovereign` Following the hurricane, there was a 72% drop in the amount of Puerto Ricans identifying as white. Did all the white people move away? Nope - they just no longer thought of themselves as white. The significant shift can be largely attributed to Puerto Ricans feeling as though they had been neglected by the US government in their inadequate response to the hurricane, and as such, redefined themselves as Latinx. That's an entire population collectively deciding to change their racial identity, and  gives an example of just how dynamic - and unpredictable - identity can be.
# 
...
#   "Intersectionality" by Sylvia Duckworth, licensed under a `CC-BY-NC-ND`<https://creativecommons.org/licenses/by-nc-nd/2.0/> license.
#
...
# 
# For example, let us consider a quote from Sojourner Truth - an enslaved person at the time. :footcite:`npsSojournerTruth`
# 
...
# --------------------
#
#   **Critical race theory** is an iterative methodology that draws on the collective wisdom of activists and scholars to study and transform the relationship between race, racism, and power. --:footcite:ct:`Ford2010`
# 
# Intersectionality is one tool within the critical race theory toolkit, among many others, and there has already been some great work :footcite:`hanna2020towards` done on how critical race theory (CRT) may be applied to algorithmic fairness, such as the need for disaggregated analysis that operates on a descriptive level in order to interrogate the most salient aspects of race for an algorithmic system. A central element of CRT, especially as it relates to ML systems, echoes a theme we have already discussed extensively: **how we define a group of people matters.** The power to define a group of people matters, and we need to question our assumptions about what makes people have something in common, and how that might affect our ability to compare populations.
# 
# The practical applications to machine learning
# ==============================================
#
# In practice, the focus on these intersectional comparisons within the ML field has been on protected groups :footcite:`justiceCanadianHuman`, such as:
#
...
#
# A case study: :footcite:ct:`obermeyer2019dissecting`
# -------------------------------------------------------------------------------------
#
...
# 
# A similar issue is presented by Buolamwini and Gebru in the gender shades project, which analyzed fairness issues in facial detection software and found that the AI system led to larger errors for dark-skinned women than for other groups.:footcite:`buolamwini2018gender`
# 
...
# ----------
#
# Our case scenario for this practice component of the tutorial is based on the finding that **Black patients (and men in particular) are diagnosed with schizophrenia at a higher rate than other demographic groups** (for example, white men). :footcite:`Olbert2018`
# 
...
# *   variables that are much more complex (e.g., race or ethnicity coded in a multitude of ways, :footcite:`Maslej2022` with many individuals having mixed racial backgrounds, psychosocial variables never cleanly separate into *stable* or *unstable* housing or *yes* or *no* for delay (in reality, these constructs are rarely even captured, and they may be inferred, e.g., based on location or income)
