...
# 
# This notebook has three main learning objectives. After this tutorial, we hope that you will be able to:
#
# 1. Think critically about how populations can be defined and how this relates to the measurement, identification, and interpretation of health inequities
# 2.   Explain the advantages of an intersectional approach to research
# 3.   Conduct an intersectional bias assessment of simulated psychiatric data, based on a finding from the psychiatric literature
#
...
# There are [3 general pillars](https://informatics.bmj.com/content/29/1/e100459) of fairness as it pertains to ML: Transparency, Inclusion, and Impartiality.
# 
...
# Although all aspects of fairness are equally important, this tutorial will focus on impartiality. Specifically, we aim to examine fairness through the scope of intersectionality, which was originally coined by [Kimberle Crenshaw](https://time.com/5786710/kimberle-crenshaw-intersectionality/):
# 
# > A framework for understanding how different forms of inequality (eg., gender and race) often operate together and exacerbate each other.
# 
...
# When we do a fairness assessment, we need to decide which groups of people to compare to identify whether there is some kind of fairness-related harm occuring.
# 
# The key question in being able to do so is this: **Who and what is a population?** It may seem like this question is trivial, and has a clear-cut meaning in no need of further clarification. However, researchers like Nancy Krieger have pointed out that a clear notion of "population" is [rarely defined](https://www.ncbi.nlm.nih.gov/pmc/articles/PMC3530737/), despite its centrality to fields like ML fairness.
# 
...
# - What is something that your whole group has in common?
#   - For example, on a soccer team, this might be the fact that everyone plays soccer.
# - What is something that some of your group has in common?
#   - Going off the same soccer team example, perhaps some of the team identifies as boys, while some of the team identifies as girls.
# - What is something that makes each of you unique?
#   - Perhaps everyone is a different height, or everyone grew up in a different neighborhood.
# 
# 
...
# In 2011, [a paper](https://www.ncbi.nlm.nih.gov/pmc/articles/PMC3149201/) studying the epidemiology of cancer noted that "early onset ER negative tumors also develop more frequently in Asian Indian and Pakistani women and in women from other parts of Asia, although not as prevalent as it is in West Africa."
# 
# 
...
# 
# 
# However, Kreiger points out that the cancer incidence rates used to arrive at this conclusion are based on:
...
# Much of [Krieger's work](https://www.ncbi.nlm.nih.gov/pmc/articles/PMC3530737/)
# stands in contrast to the conventional definition of population, which is limited to an understanding of populations as statistical objects, rather than of substantive beings. This definition is as follows:
# 
...
# 
# This idea that humans have **innate and comparable** was largely derived from  Adolphe Quetelet's invention of the [**"Average Man,"**](https://academic.oup.com/ndt/article/23/1/47/1923176) establishing the notion of a population mean.
# 
...
# > "For a star, the location of the mean referred to the location of a singular real object, whereas for a population, the location of a population mean **depended on how the population was defined."**
# 
# 
# 
# 
...
# 
# 
# This article is hardly unique in this problem - body mass index (BMI) has become a ubiquitous way of defining obesity, but comparing individuals on the basis of their BMI is problematic: the scale was built by and for white populations, leading to an overestimation of health risks for [Black individuals](https://www.sciencedaily.com/releases/2009/06/090611142407.htm), and an underestimation of health risks for [asian individuals](https://academic.oup.com/ptj/article/83/3/276/2857568?login=false). Even more interestingly, BMI was [never meant to be a way of measuring health at an individual level](https://www.psychologytoday.com/us/blog/the-gravity-weight/201603/adolphe-quetelet-and-the-evolution-body-mass-index-bmi), but rather as a tool for studying population-level health.
# 
# 
# 
# ---
...
# This point is especially important because of how **heterogenous humans are**, across space and time, both between groups and within groups. Someone who has written extensively about this in relation to ML, disability, and design is Jutta Treviranus.
# 
# Treviranus points out that the further you get from a population mean, the more heterogeneous the population gets (as [seen below](https://idrc.ocadu.ca/ideas/inclusive-design-the-bell-curve-the-starburst-and-the-virtuous-tornado/), the dots get spaced farther apart). And as you move away from this "average mean" - not only do people get more diverse from one another, but solutions that are designed with the population mean in mind are increasingly ineffective.
# 
...
# Given these limitations, let's consider an alternative understanding of a population that [Krieger proposes](https://www.ncbi.nlm.nih.gov/pmc/articles/PMC3530737/):
# 
...
# 
# 
# Krieger's view emphasizes that **identity is not a fixed attribute (like a star), but rather a fluid social process**. A great [example](https://anthrosource.onlinelibrary.wiley.com/doi/abs/10.1111/aman.13601) that illustrates the degree to which identity can change can be seen in the racial identity of Puerto Ricans in the aftermath of Hurricane Maria. Following the hurricane, there was a 72% drop in the amount of Puerto Ricans identifying as white. Did all the white people move away? Nope - they just no longer thought of themselves as white. The significant shift can be largely attributed to Puerto Ricans feeling as though they had been neglected by the US government in their inadequate response to the hurricane, and as such, redefined themselves as Latinx. That's an entire population collectively deciding to change their racial identity, and  gives an example of just how dynamic - and unpredictable - identity can be.
# 
# 
...
# Note that although this intersectional approach has only recently been applied to ML, it is not a novel concept. Rather, it’s a well-established idea that stretches back nearly 200 years to the Black feminist and the suffragette movement, when white women were fighting for the right to vote, but Black women seemed to be left out of this call for reform. There were a range of Black activists and scholars who responded to this - wondering "What about me? Don't I matter?"
# 
# For example, let us consider a [quote](https://www.nps.gov/articles/sojourner-truth.htm#:~:text=I%20could%20work%20as%20much,ain't%20I%20a%20woman%3F) from Sojourner Truth - an enslaved person at the time.
# 
...
# > An iterative methodology that draws on the collective wisdom of activists and scholars to study and transform the relationship between race, racism, and power
# 
# -[Ford and Airihenbuwa](https://www.ncbi.nlm.nih.gov/pmc/articles/PMC2837428/)
# 
# Intersectionality is one tool within the critical race theory toolkit, among many others, and there has already been some [great work](https://arxiv.org/pdf/1912.03593.pdf) done on how critical race theory (CRT) may be applied to algorithmic fairness, such as the need for disaggregated analysis that operates on a descriptive level in order to interrogate the most salient aspects of race for an algorithmic system. A central element of CRT, especially as it relates to ML systems, echoes a theme we have already discussed extensively: **how we define a group of people matters.** The power to define a group of people matters, and we need to question our assumptions about what makes people have something in common, and how that might affect our ability to compare populations.
# 
# 
# 
...
# In practice, the focus on these intersectional comparisons within the ML field has been on [protected groups](https://laws-lois.justice.gc.ca/eng/acts/h-6/fulltext.html), such as:
...
# A case study: [Obermeyer et al.](https://www.science.org/doi/10.1126/science.aax2342)
# -------------------------------------------------------------------------------------

# To highlight the way that fairness harms has been evaluated in health care, we will examine a study by Obermeyer and colleagues, which examined a risk assessment algorithm for health insurance across approximately **50000 primary care patients**. This algorithm was used to identify individuals who required further follow-up for complex care.
# 
# 
...
# 
# A similar issue is presented by Buolamwini and Gebru in the [gender shades](http://proceedings.mlr.press/v81/buolamwini18a/buolamwini18a.pdf) project, which analyzed fairness issues in facial detection software and found that the AI system led to larger errors for dark-skinned women than for other groups.
# 

# Our applied case scenario
# =========================
...
# Our case scenario for this practice component of the tutorial is based on the [finding](https://pubmed.ncbi.nlm.nih.gov/29094963/) that **Black patients (and men in particular) are diagnosed with schizophrenia at a higher rate than other demographic groups** (for example, white men).
# 
# 
...
# 
# But first, they must provide some evidence to stakeholders that this classifier works well in diagnosing patients, so they have a test sample of 1000 patients, on which they must evaluate the model. They argue that their classifier works very well, based on some impressive sensitivity and specificity values. The stakeholders are not fully convinced (thinking that the executives may be a little too eager to get this model deployed), so they hire us (an independent consulting firm) to evaluate the model.
# 
...
def grouppivot(labelgroup, yvalue, dataset, SZonly = False):
  ...
  grouped = (dataset.groupby([labelgroup])[yvalue]
                    .value_counts(normalize=True)
                    .rename('percentage')
                    .reset_index())

  pivot = pd.pivot_table(grouped, index=labelgroup, columns=yvalue, values='percentage', aggfunc='sum')
  return pivot
...
# *   variables that are much more complex (e.g., race or ethnicity coded in a [multitude](https://ebooks.iospress.nl/doi/10.3233/SHTI220281) of ways, with many individuals having mixed racial backgrounds, psychosocial variables never cleanly separate into *stable* or *unstable* housing or *yes* or *no* for delay (in reality, these constructs are rarely even captured, and they may be inferred, e.g., based on location or income)
...
# 
# This performance is another overly-simplistic component of our case scenario. In real-world settings, and for psychiatric outcomes in particular, ML rarely achieves performance this good, partly because training features are never as clearly related to outcomes as complex as diagnoses of Schizophrenia or Affective Disorder. These conditions are often heterogeneous, meaning that individuals with the same disorder can have very different symptom profiles. Symptoms of schizophrenia (such as a lack of emotion and motivation, cognitive impairment, and even psychosis) can overlap with symptoms of depression. Another reason ML often falls short in predicting psychiatric outcomes is when these outcomes tend to be rare (such as in the case of [suicide](https://jamanetwork.com/journals/jamapsychiatry/fullarticle/2727387?casa_token=lnM5oR156VMAAAAA:diFvNh309XQO6SbiyKXfHWcxl2f17kzIJmqLONMqzaP8UTNDXRuQQE3tJVdXBn3ugLwkimFF9gM)). Although our simulated data is fairly balanced with respect to the two diagnoses, in reality, Schizophrenia is much less common than Affective Disorder. If collected naturally based on different patients seen at a health system, our classes would be very imbalanced.

# Fairness
# ========
#
# While seemingly naive, many ML algorithms deployed in the past have indeed failed to properly address fairness. Perhaps the most infamous example of this is the [COMPAS algorithm](https://www.propublica.org/article/machine-bias-risk-assessments-in-criminal-sentencing) for prediction criminal recidivism, where Black defendants were found to have higher risk scores, a higher false positive rate, and a lower false negative rate compared to white defendants. In other applications of ML, the lack of fairness concerns proves pervasive. An [audit](http://proceedings.mlr.press/v81/buolamwini18a/buolamwini18a.pdf) of three facial recognition software from IBM and Microsoft found the error rate for darker-skinned females to be 34% higher than for lighter-skinned males. When translating into a gendered language, [Google Translate](https://blog.google/products/translate/reducing-gender-bias-google-translate/) skews towards masculine translations for words like “strong” or “doctor,” while skewing feminine for words like “beautiful” or “nurse.”
# 
...
# --------------------
#
# Many fairness metrics are calculated from basic performance metrics, like sensitivity, specificity, TPR, FPR, etc. There are many performance metrics available, so we need to select one that is most relevant to our task.

# One common (and arguably most successful to-date) application of ML in healthcare is in the diagnosis of tumours from medical images (like chest x-rays) as being cancerous or non-cancerous.
# 
# TODO ADD IMAGE
#
...
# 
# There is a [study](https://pubmed.ncbi.nlm.nih.gov/34893776/) suggesting that, when diagnosing the nature of tumours from medical images, false negative rates are higher for disadvantaged or underserved groups defined by intersecting features (e.g., low-income, Black women). This is an excellent, real-world example of how the application of ML in healthcare can *amplify harms*, if deployed without concern for fairness.
#
...
# 
# Another decision is related to the metric – we could have also considered some other performance metrics. Selection rate, for example, refers to the proportion of positive predictions between the two groups. ($\text{Selection rate}: \frac{TP + FP}{n}$).
# 
...
# 
# 
# Critically, there are no readily-agreed upon definitions and understandings of fairness. There are over 70 definitions of fairness, many of which conflict with each other, making it **impossible to simultaneously satisfy all possible metrics for fairness.** For a more comprehensive review on fairness metrics, consult [this](https://fairware.cs.umass.edu/papers/Verma.pdf) 2018 review by Verma and Robin.

# Fairness assessment
# -------------------

# This code uses the [Fairlearn](https://fairlearn.org/) package in Python, an open-source tool that can assist with fairness assessment, which we install here.
...
# 
# Finally, the performance of a ML model cannot be analyzed in isolation, but rather in comparison between different demographic subgroups. In order to do this, a reference group is required. As we explained, we will use white individuals as a reference group, but this assessment could be conducted with any reference group. It is important to note that this approach can be problematic; there are concerns that using white groups as reference groups centers their perspectives or experiences, and represents other groups as *other* or outliers. Keeping this in mind, we should carefully consider which group we use as a reference and the implications of this choice.
...
def f(truelabels, predictions):
  ...
  # Define a MetricFrame using a FPR of the defined sensitive features, using
...
# However, we suspect this bias might only extend to identities defined by the intersecting features of sex and race (i.e., [Black men](https://ps.psychiatryonline.org/doi/full/10.1176/appi.ps.201800223?mod=article_inline&utm_source=TrendMD&utm_medium=cpc&utm_campaign=Psychiatric_Services_TrendMD_0)).
# 
...
def intersectionalf(truelabels, predictions):
  ...
  sensitive = pd.DataFrame(np.stack([test.Race, test.Sex], axis=1),
                          columns=['Race', 'Sex'])
...
model2 = LogisticRegression(penalty = "elasticnet", max_iter=1000,
                           solver = "saga", l1_ratio=1)
...
scaled_weights = pd.DataFrame(scaler.fit_transform(abs(weights)), trainx.columns, columns= weights.columns)
...
sns.barplot(x = "intersect", y = "Rumination", data = test.loc[test.Diagnosis == 0], ax = axs[1],
...
sns.barplot(x = "intersect", y = "Tension", data = test.loc[test.Diagnosis == 0], ax = axs[1],
            order = ["BlackF", "WhiteF", "HispanicF", "AsianF", "BlackM", "WhiteM", "HispanicM", "AsianM"])
