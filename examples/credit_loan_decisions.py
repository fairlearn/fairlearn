...
# %% 
# As the data is read in-memory, we will change the column :param:`PAY_0` to :param:`PAY_1` to make the naming more consistent with the naming of the other columns. In addition, the target variable :param:`default payment next month` is changed to `default` to reduce verbosity.
...
# %% [markdown]
# From the `dataset description <https://archive.ics.uci.edu/ml/datasets/default+of+credit+card+clients>`_, we see there are three categorical features:
...
# %%
# A secondary harm that is somewhat unique to credit lending decisions is the long-term impact on an individual's credit score. In the United States, a `FICO credit score <https://www.investopedia.com/terms/c/credit_score.asp>`_ is a number between 300 and 850 that represents a customer's *creditworthiness*. An applicant's *credit score* is used by many financial institutions for lending decisions. An applicant's *credit score* usually increases after a successful repayment of a loan and decreases if the applicant fails to repay the loan.
#
...
# %% 
# One key assumption here is we assume that *false positives* and *false negatives*
# have the equally adverse costs to each group. In practice, we would develop some 
# weighting mechanism to assign a weight to each *false negative* and *false positive* event.
...
# %% 
# In this section, we will use the *reductions* approach of `Agarwal et. al (2018) <https://arxiv.org/pdf/1803.02453.pdf>`_ to produce models that satisfy the fairness constraint without needing access to the sensitive features at deployment time.
#
...
# %%
# In practice, we recommend choosing smaller values of :param:`epsilon` on the order of the _square root_ of the number of samples in the training dataset:
...
# %% 
# Here, we can see all the inner models learned for each value of :param:`epsilon`.
...
for index, row in performance_df_masked.iterrows():
    ...
    plt.text(bal_error+0.001, eq_odds_diff+0.0001, f"Eps: {epsilon_}, Idx: {int(index_)}", fontsize=10)
...
# %% 
# A key facet of Responsible Machine Learning is responsible documentation practices. `Mitchell et al. (2019) <https://arxiv.org/abs/1810.03993>`_ proposed the model card framework for documentating and reporting model training details and deployment considerations. 
#
# A *model card* contains sections for documenting training and evaluation dataset descriptions, ethical concerns, and quantitative evaluation summaries. In practice, we would ideally create a model card for our model before deploying it in production. Although we will not be producing a model card in this case study, interested readers can learn more about creating model cards using the *Model Card Toolkit* from the `Fairlearn PyCon 2022 tutorial <https://colab.research.google.com/github/LeJit/talks/blob/pycon_2022/2022_pycon/pycon-2022-students.ipynb>`_.
